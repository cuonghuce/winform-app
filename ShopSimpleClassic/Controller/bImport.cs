...
namespace ShopSimpleClassic.Controller
{
    public class bImport
    {
        ...
        public IEnumerable<Import> List(string text, DateTime dateFrom, DateTime dateTo, bool isDate,
        ...
        {
            return ShopSimpleClassic.Library.Lib.CreateKey("NH", true);
        }
        ...
        public Import Detail(string code)
        {
        ...
        public int TotalBillToday()
        {
            try
            {
                ...
                return db.Imports.Count(i => i.Date.Day == dateTime.Day && i.Date.Month == dateTime.Month && i.Date.Year == dateTime.Year);
            }
            catch
            {
                return 0;
        ...
        public int TotalBillMonth()
        {
            ...
            {
                var dateTime = DateTime.Now;
                return db.Imports.Count(i => i.Date.Month == dateTime.Month && i.Date.Year == dateTime.Year);
            }
            catch
            {
                return 0;
                                        ...
                                        string priceFrom, string priceTo, bool isPrice)
        {
            ...
            return lst.ToList().FindIndex(i => i.ImportCode.Equals(code));
        }

        private IEnumerable<Import> getList(string text, string priceFrom, string priceTo, DateTime dateFrom, DateTime dateTo)
        {
        ...
        private IEnumerable<Import> getList(string text, string priceFrom, string priceTo)
        {
            int pFrom = Convert.ToInt32(string.IsNullOrEmpty(priceFrom) ? "0" : priceFrom);
            int pTo = Convert.ToInt32(string.IsNullOrEmpty(priceTo) ? "0" : priceTo);
