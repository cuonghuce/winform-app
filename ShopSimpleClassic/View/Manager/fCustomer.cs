...
namespace ShopSimpleClassic.View.Manager
{
    public partial class fCustomer : Form
    {
        ...
        private void tbExport_Click(object sender, EventArgs e)
        {
            try
            {
                Lib.ExportExcel(dgvList, "Khách Hàng");
            }
            catch (Exception ex)
            {
                ShowMess.Exception(ex);
        ...
        private void btSearch_Click(object sender, EventArgs e)
        {
            ...
            if (new bCustomer().TotalRows(tbSearch.Text.Trim()) == 0)
            {
        ...
        {
            int rows = new bCustomer().TotalRows(tbSearch.Text.Trim());
        ...
        private void loadList()
        {
            ...
            // Lấy danh sách Customer từ bộ xử lý bCustomer dựa trên từ khóa tìm kiếm, trang hiện tại và kích thước trang
            IEnumerable<Customer> data = new bCustomer().List(tbSearch.Text.Trim(), currentPage - 1, pageSize);

            dgvList.Rows.Clear();

            // Thêm dữ liệu Customer vào DataGridView
            dgvList.Rows.AddRange(data.Select((i, count) => new DataGridViewRow
            {
                Cells =
                {
                    ...
                    new DataGridViewTextBoxCell { Value = i.Phone }, // Ô hiển thị mã Customer
                    new DataGridViewTextBoxCell { Value = i.Name } // Ô hiển thị tên Customer
                }
            }).ToArray());
        ...
        {
            var data = new bCustomer().Detail(codeSelected);

            if (data == null) return;

            tbCode.Text = data.Phone;
        ...
        private void AddOrUpdateData(bool isAdd)
        {
            ...
            if (isAdd && new bCustomer().IsExists(_data.Phone))
            {
                ShowMess.Error__AlreadyExist(mess); // Hiển thị thông báo lỗi: Customer đã tồn tại
                return;
            }
            else if (!isAdd && !new bCustomer().IsExists(_data.Phone))
            {
                ShowMess.Error__NotExists(mess); // Hiển thị thông báo lỗi: Customer không tồn tại
                return;
            }

            bool result = isAdd ? new bCustomer().Add(_data) : new bCustomer().Update(_data); // Thêm mới hoặc cập nhật Customer
            ...
            {
                setSelectInList(_data.Phone); // Xử lý trường hợp từ khoá tìm kiếm khi có từ tương ứng
            }
        ...
        private void deleteData()
        {
            ...
            // Tạo thông điệp thông báo từ dữ liệu Customer
            string mess = $"{_data.Phone}: {_data.Name}";

            // Kiểm tra xem Customer có tồn tại hay không
            if (!new bCustomer().IsExists(_data.Phone))
            {
                // Hiển thị thông báo lỗi: Customer không tồn tại
                ShowMess.Error__NotExists(mess);
                return;
            }

            // Xóa Customer và kiểm tra kết quả xóa
            bool result = new bCustomer().Delete(_data.Phone);

            if (!result)
            {
                // Hiển thị thông báo lỗi: Xóa Customer không thành công
            ...
            if (listPage.Count == 0)
            {
                // Hiển thị thông báo thành công: Xóa Customer thành công
        ...
        private void setSelectInList(string code)
        {
            ...
            int index = new bCustomer().IndexRows(code, tbSearch.Text.Trim());
        ...
        private bool packageData()
        {
            ...
            // Tạo một đối tượng Customer mới và gán giá trị từ các trường dữ liệu vào đó
            _data = new Customer
            {
