...
namespace ShopSimpleClassic.View.Manager
{
    public partial class fEmployee : Form
    {
        ...
        private void tbExport_Click(object sender, EventArgs e)
        {
            try
            {
                Lib.ExportExcel(dgvList, "Nhân Viên");
            }
            catch (Exception ex)
            {
                ShowMess.Exception(ex);
        ...
        private void btSearch_Click(object sender, EventArgs e)
        {
            ...
            if (new bUser().TotalRows(tbSearch.Text.Trim()) == 0)
            {
        ...
        {
            int rows = new bUser().TotalRows(tbSearch.Text.Trim());
        ...
        private void loadList()
        {
            ...
            // Lấy danh sách User từ bộ xử lý bUser dựa trên từ khóa tìm kiếm, trang hiện tại và kích thước trang
            IEnumerable<User> data = new bUser().List(tbSearch.Text.Trim(), currentPage - 1, pageSize);

            dgvList.Rows.Clear();

            // Thêm dữ liệu User vào DataGridView
            dgvList.Rows.AddRange(data.Select((i, count) => new DataGridViewRow
            {
                Cells =
                {
                    new DataGridViewTextBoxCell { Value = startIndex + count },
                    new DataGridViewTextBoxCell { Value = i.Username },
                    new DataGridViewTextBoxCell { Value = i.Name },
                    new DataGridViewTextBoxCell { Value = i.Phone },
                    new DataGridViewTextBoxCell { Value = i.Address }
                }
            }).ToArray());
        ...
        {
            var data = new bUser().Detail(codeSelected);
        ...
        private void AddOrUpdateData(bool isAdd)
        {
            ...
            if (isAdd && new bUser().IsExists(_data.Username))
            {
                ShowMess.Error__AlreadyExist(mess); // Hiển thị thông báo lỗi: User đã tồn tại
                return;
            }
            else if (!isAdd && !new bUser().IsExists(_data.Username))
            {
                ShowMess.Error__NotExists(mess); // Hiển thị thông báo lỗi: User không tồn tại
                return;
            }

            bool result = isAdd ? new bUser().Add(_data) : new bUser().Update(_data); // Thêm mới hoặc cập nhật User
        ...
        private void deleteData()
        {
            ...
            // Tạo thông điệp thông báo từ dữ liệu User
            string mess = $"{_data.Username}: {_data.Name}";

            // Kiểm tra xem User có tồn tại hay không
            if (!new bUser().IsExists(_data.Username))
            {
                // Hiển thị thông báo lỗi: User không tồn tại
                ShowMess.Error__NotExists(mess);
                return;
            }

            // Xóa User và kiểm tra kết quả xóa
            bool result = new bUser().Delete(_data.Username);

            if (!result)
            {
                // Hiển thị thông báo lỗi: Xóa User không thành công
            ...
            if (listPage.Count == 0)
            {
                // Hiển thị thông báo thành công: Xóa User thành công
        ...
        private void setSelectInList(string code)
        {
            ...
            int index = new bUser().IndexRows(code, tbSearch.Text.Trim());
        ...
        private bool packageData()
        {
            ...
            // Tạo một đối tượng User mới và gán giá trị từ các trường dữ liệu vào đó
            _data = new User
            {
