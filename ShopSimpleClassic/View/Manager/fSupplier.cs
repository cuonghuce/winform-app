...
namespace ShopSimpleClassic.View.Manager
{
    public partial class fSupplier : Form
    {
        ...
        private void tbExport_Click(object sender, EventArgs e)
        {
            try
            {
                Lib.ExportExcel(dgvList, "Nhà Cung Cấp");
            }
            catch (Exception ex)
            {
                ShowMess.Exception(ex);
        ...
        private void btSearch_Click(object sender, EventArgs e)
        {
            ...
            if (new bSupplier().TotalRows(tbSearch.Text.Trim()) == 0)
            {
        ...
        {
            int rows = new bSupplier().TotalRows(tbSearch.Text.Trim());
        ...
        private void loadList()
        {
            ...
            // Lấy danh sách Supplier từ bộ xử lý bSupplier dựa trên từ khóa tìm kiếm, trang hiện tại và kích thước trang
            IEnumerable<Supplier> data = new bSupplier().List(tbSearch.Text.Trim(), currentPage - 1, pageSize);

            dgvList.Rows.Clear();

            // Thêm dữ liệu Supplier vào DataGridView
            dgvList.Rows.AddRange(data.Select((i, count) => new DataGridViewRow
            {
                Cells =
                {
                    new DataGridViewTextBoxCell { Value = startIndex + count },
                    new DataGridViewTextBoxCell { Value = i.SupplierCode },
                    new DataGridViewTextBoxCell { Value = i.Name },
                    new DataGridViewTextBoxCell { Value = i.Email },
        ...
        {
            var data = new bSupplier().Detail(codeSelected);

            if (data == null) return;

            tbCode.Text = data.SupplierCode;
        ...
        private void AddOrUpdateData(bool isAdd)
        {
            ...
            string mess = $"{_data.SupplierCode}: {_data.Name}";

            // Kiểm tra tồn tại cho thêm mới và cập nhật
            if (isAdd && new bSupplier().IsExists(_data.SupplierCode))
            {
                ShowMess.Error__AlreadyExist(mess); // Hiển thị thông báo lỗi: Supplier đã tồn tại
                return;
            }
            else if (!isAdd && !new bSupplier().IsExists(_data.SupplierCode))
            {
                ShowMess.Error__NotExists(mess); // Hiển thị thông báo lỗi: Supplier không tồn tại
                return;
            }

            bool result = isAdd ? new bSupplier().Add(_data) : new bSupplier().Update(_data); // Thêm mới hoặc cập nhật Supplier
            ...
            {
                setSelectInList(_data.SupplierCode); // Xử lý trường hợp từ khoá tìm kiếm khi có từ tương ứng
            }
        ...
        private void deleteData()
        {
            ...
            // Tạo thông điệp thông báo từ dữ liệu Supplier
            string mess = $"{_data.SupplierCode}: {_data.Name}";

            // Kiểm tra xem Supplier có tồn tại hay không
            if (!new bSupplier().IsExists(_data.SupplierCode))
            {
                // Hiển thị thông báo lỗi: Supplier không tồn tại
                ShowMess.Error__NotExists(mess);
                return;
            }

            // Xóa Supplier và kiểm tra kết quả xóa
            bool result = new bSupplier().Delete(_data.SupplierCode);

            if (!result)
            {
                // Hiển thị thông báo lỗi: Xóa Supplier không thành công
            ...
            if (listPage.Count == 0)
            {
                // Hiển thị thông báo thành công: Xóa Supplier thành công
        ...
        private void setSelectInList(string code)
        {
            ...
            int index = new bSupplier().IndexRows(code, tbSearch.Text.Trim());
        ...
        private bool packageData()
        {
            ...
            // Tạo một đối tượng Supplier mới và gán giá trị từ các trường dữ liệu vào đó
            _data = new Supplier
            {
